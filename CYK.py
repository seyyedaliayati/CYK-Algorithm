...
def check_grammer(Grammer):
    ...
    for Notation in Grammer:
        ...
        if len(Notation[1])>2 or len(Notation[1]) == 0 : return False
        if len(Notation[1]) == 1 and not(Notation[1][0].islower()): return False
        if len(Notation[1]) == 2 :
            if not(Notation)[1][0].isupper() or not(Notation)[1][1].isupper() : return False
            for i in range(2):
                if Notation[1][i] not in Variables:
                    Variables[Notation[1][i]] = "Undefiend"
    for Value in Variables.values():
        if Value == "undefiend" : return False
    return True      
